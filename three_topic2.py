# -*- coding: utf-8 -*-
"""
Created on Tue Oct 25 17:48:24 2022
...
def ae_adversarial_loss(X, Z, x, x_hat, d_z0, d_z1, d_z2,first_order, alpha): #change
  ...
  shape1=d_z2.shape
  tuple1=(shape1[0],shape1[1])
  full_tensor1 = torch.full(tuple1, 2)  
  ...
  adversarial_loss = tf.reduce_sum(tf.keras.losses.binary_crossentropy(tf.ones_like(d_z0), d_z0)) + \
                     tf.reduce_sum(tf.keras.losses.binary_crossentropy(tf.zeros_like(d_z1), d_z1)) + \
                         tf.reduce_sum(tf.keras.losses.binary_crossentropy(full_tensor1, d_z2))
...
def disc_loss_function(d_z0, d_z1,d_z2):  # change
  ...
  tuple1=(shape1[0],shape1[1])
  full_tensor1 = torch.full(tuple1, 2)
  loss_two = tf.keras.losses.binary_crossentropy(full_tensor1, d_z2)
  #loss_two = tf.keras.losses.binary_crossentropy(tf.twos_like(d_z2), d_z2)

  #return tf.cast(loss_zero, tf.float32) + tf.cast(loss_one, tf.float32) + tf.cast(loss_two, tf.float32)
  return tf.cast(loss_zero, tf.float32) + tf.cast(loss_one, tf.float32) 
     
def train_step(X, x0, x1, x2, encoder, decoder, auto_encoder, discriminator, ae_optimizer, disc_optimizer, first_order, alpha):
  with tf.GradientTape() as ae_tape, tf.GradientTape() as disc_tape:
    ...
    d_z2 = discriminator(z2, training=True)
    
    x0_hat = decoder(z0, training=True)
    x1_hat = decoder(z1, training = True)
    x2_hat = decoder(z2, training = True)
       
    ae_loss = ae_adversarial_loss(X, Z, tf.concat([x0, x1,x2], 0), tf.concat([x0_hat, x1_hat,x2_hat], 0), d_z0, d_z1, d_z2, first_order, alpha)
    disc_loss = disc_loss_function(d_z0, d_z1,d_z2)
  ...
  ae_acc = ae_accuracy(tf.concat([x0, x1,x2], 0), tf.concat([x0_hat, x1_hat,x2_hat], 0))
  
  return tf.reduce_mean(ae_loss), ae_acc, tf.reduce_mean(disc_loss)


def pretrain_step_disc(x0, x1,x2, encoder, discriminator, disc_pre_optimizer):
...
def pretrain_disc(X, idxs_zeros, idxs_ones,idxs_twos, encoder, discriminator, disc_pre_optimizer):
  
  EPOCHS = 40

  np.random.shuffle(idxs_zeros)
  np.random.shuffle(idxs_ones)
  np.random.shuffle(idxs_twos)
  Batch_size = 50
  ...
  for epoch in range(EPOCHS):
    for batch_idx in range(0, len(idxs_ones), Batch_size):
      ...
      selected_twos = idxs_twos[batch_idx : batch_idx + Batch_size]
      
      x0 = X[selected_zeros]
      x1 = X[selected_ones]
      x2= X[selected_twos]

      pretrain_step_disc(x0, x1, x2,encoder, discriminator, disc_pre_optimizer)


def adversarial_train(X, idxs_zeros, idxs_ones,idxs_twos, encoder, decoder, auto_encoder, discriminator, ae_optimizer, disc_optimizer, first_order, alpha):
    EPOCHS = 500
    ...
    for epoch in range(EPOCHS):
        for batch_idx in range(0, len(idxs_ones), Batch_size):
          ...
          x2= X[selected_twos]
          ### Joint Training
          train_step(X, tf.cast(x0, tf.float32), tf.cast(x1, tf.float32),tf.cast(x2, tf.float32), encoder, decoder,
                                                     auto_encoder, discriminator, ae_optimizer, disc_optimizer, first_order, alpha)
          
          
def get_seeds(N_CLUS, embedding, nodes, labels, nodes_zero, nodes_one, nodes_two,strategy, n_seeds):
    '''
    ...
    for i in range(N_CLUS):
        ...
        if strategy == 'nearest':
          ...
          two_in_clus = embedding[np.logical_and(cluster_number == i, labels == 2)]
          two_inds = nodes[np.logical_and(cluster_number == i, labels == 2)] 
          ...
          added_to_two = 0
          if len(two_in_clus) != 0:
              model_on_two = KMeans(n_clusters=1)
              model_on_two.fit(two_in_clus)
              center_two = model_on_two.cluster_centers_
              
              sorted_distance_two = np.array(sorted([[np.sqrt(np.sum(np.power(center_two - two_in_clus[j], 2))), j] for j in range(len(two_in_clus))]))
              seed_ids[i].extend([two_inds[int(i)] for i in sorted_distance_two[:portion_two, 1]])
              
              added_to_two = len(seed_ids[i]) - added_to_zero
              assert added_to_two == portion_two    
              
          assert n_seeds == added_to_zero + added_to_one + added_to_two
...
def repeated_IC(G, nodes_zero, nodes_one,nodes_two,seeds, seeds_type, n_expr, imp_prob, recover_prob = 0, remove = 0):
  zeros_count = []
  ones_count = []
  twos_count=[]
  total_count = []
  ...
  #fraction_two = np.round(np.mean(twos_count) / len(nodes_two), 3)
...
def get_graph_real():
  ...
  attr_df.columns = ['id', 'numEvents', 'numGroups', 'Topic']
...
def get_nodes_labels_real(input_G):
  ...
  data.columns = ['id', 'numEvents', 'numGroups', 'Topic']
  ...
  nodes_two = nodes[labels == 2]
  
  return nodes_zero, nodes_one,nodes_two, labels


def get_idxs(n, nodes_zero, nodes_one,nodes_two):
  
  diff_size1 = np.abs(len(nodes_one) - len(nodes_zero))
  diff_size2 = np.abs(len(nodes_one) - len(nodes_two))
  diff_size3 = np.abs(len(nodes_zero) - len(nodes_two))
  diff_size=diff_size1+diff_size2+diff_size3
  ...
  if len(nodes_two) < len(nodes_one):
    if diff_size > len(nodes_zero):
      rep_policy = True
    two_draws = np.random.choice(nodes_zero,size=diff_size, replace=rep_policy)
    idxs_twos = np.concatenate((idxs_twos, two_draws))  
  ...
  print(idxs_twos)
  print("================")
  return np.arange(n), idxs_zeros, idxs_ones,idxs_twos
    


def print_edges(G, nodes_zero, nodes_one,nodes_two):
  zero_edges = 0
  one_edges = 0
  #two_edges = 0
  accross_edges = 0
...
nodes_zero, nodes_one, nodes_two, labels = get_nodes_labels_real(input_G)

#print_edges(G, nodes_zero, nodes_one)

# 3. Getting the idxs suitable for training.
idxs, idxs_zeros, idxs_ones,idxs_twos = get_idxs(n, nodes_zero, nodes_one,nodes_two)
...
pretrain_disc(X, idxs_zeros, idxs_ones, idxs_twos,encoder, discriminator, pre_optimizer_disc)
# print('6')
...
adversarial_train(X, idxs_zeros, idxs_ones,idxs_twos, encoder, decoder, auto_encoder, discriminator, ae_optimizer, disc_optimizer, first_order_imp, alpha)
...
for N_CLUS in N_CLUSs:
  print("Number of cluster",N_CLUSs)      
  for n_seeds in n_seedss:
    print("Number of seeds",n_seedss)
    ...
    fair_seeds = get_seeds(N_CLUS, fair_embds, idxs, labels, nodes_zero, nodes_one,nodes_two, strategy, n_seeds)
    print("====Fair seeds====")
    print(fair_seeds)
    print("==================")
    pre_seeds = get_seeds(N_CLUS, pre_embds, idxs, labels, nodes_zero, nodes_one, nodes_two,strategy, n_seeds)
    
    #9. Getting the final results
    total_fair, fair_frac, zero_fair, one_fair = repeated_IC(G, nodes_zero, nodes_one, nodes_two,fair_seeds, 'fair', 2000, 0.01)
    total_pre, pre_frac, zero_pre, one_pre = repeated_IC(G, nodes_zero, nodes_one,nodes_two, pre_seeds, 'pre', 2000, 0.01)    
